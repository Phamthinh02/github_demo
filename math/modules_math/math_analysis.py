...
def Pressure_discharge(Piston_journey,Connecting_rod_length,Compression_ratio,Cylinder_diameter,Intake_pressure,Temperature):
    
    Cylinder_pressure = Intake_pressure*0.96
    SE = ((Piston_journey*20/180)*(1 + math.cos((20*math.pi/2)/180))/2) + Connecting_rod_length - math.sqrt((Connecting_rod_length**2)-(((Piston_journey*20/180)*math.sin((20*math.pi)/180))**2)/4)
    Ve = (math.pi*(Cylinder_diameter**2)*SE)/4
    Vc = (math.pi/(4*(Compression_ratio-1)))*(Cylinder_diameter**2)*Piston_journey
    Dynamic_compression_ratio = (Ve+Vc)/Vc
    n0 = (8.314/(19.806+0.002095*(Temperature+20)*(Dynamic_compression_ratio**(n-1)+1))+1)
